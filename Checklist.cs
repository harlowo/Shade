...
namespace Shade
{
    ...
    public class ModIntegrationsSystem : ModSystem
    {
        ...
        private void DoBossChecklistIntegration()
        {
            ...
            string internalName = "OnyxBoss";

            // Value inferred from boss progression, see the wiki for details
            float weight = 11.7f;

            // Used for tracking checklist progress
            Func<bool> downed = () => DownedBossSystem.downedOnyxBoss;

            // The NPC type of the boss
            int bossType = ModContent.NPCType<Content01.Npcs.OnyxExcavator.OnyxExcavatorHead>();

            // The item used to summon the boss with (if available)
            int spawnItem = ModContent.ItemType<Content01.Items.OnyxExcavatorSummon>();

            // By default, it draws the first frame of the boss, omit if you don't need custom drawing
            // But we want to draw the bestiary texture instead, so we create the code for that to draw centered on the intended location
            var customPortrait = (SpriteBatch sb, Rectangle rect, Color color) => {
                Texture2D texture = ModContent.Request<Texture2D>("Shade/OnyxExcavator/Content/NPCs/OnyxExcavator/OnyxExcavator_BossChecklist").Value;
