...
using NetEasy; //REQUIRES THE DLL AND THE .csproj TO HAVE THE HINTPATH

namespace Shade.Common.Players
{
    public class ControlsPlayer : ModPlayer
    {
        //this is used to keep track of Player controls that are otherwise not possible to keep in sync (terraria is not very flexible when it comes to right click syncing, though only for modded items, oddly enough)
        //slr is a godsend
        ...
        public override void PreUpdate()
        {
            if (Main.myPlayer == Player.whoAmI)
            {
                ...
                if (mouseListener && Vector2.Distance(mouseWorld, oldMouseWorld) > 5f) //dont send if the distance was literally so small it wouldn't even change anything
                {
