...
public class OnyxRelicTile : ModTile
{
    ...
    public virtual string RelicTextureName => "Shade/Relics/OnyxRelic1";

    // All relics use the same pedestal texture, this one is copied from vanilla
    public override string Texture => "Shade/Relics/RelicPedestal";
    ...
    {
        Main.tileShine[Type] = 200; // Responsible for golden particles
    ...
    public override void SpecialDraw(int i, int j, SpriteBatch spriteBatch)
    {
        ...
        var frame = texture.Frame(HorizontalFrames, VerticalFrames, 0, frameY);
        ...
        var offset = (float)Math.Sin(Main.GlobalTimeWrappedHourly * TwoPi / 5f);
        ...
        var scale = (float)Math.Sin(Main.GlobalTimeWrappedHourly * TwoPi / 2f) * 0.3f + 0.7f;
        var effectColor = color;
        effectColor.A = 0;
        effectColor = effectColor * 0.1f * scale;
        for (var num5 = 0f; num5 < 1f; num5 += 355f / (678f * (float)Math.PI))
        {
